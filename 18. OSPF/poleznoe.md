...
### ТИПЫ LSA
1. Router - ифнормация в LSA 1 типа содержит описание  Router ID Маршрутизатора, его интерфейсы и соседства. Так же указана стоимость каждого канала.
2. Network - Распространяется DR в сетях со множественным доступом. В LSA содержится описание всех маршрутизаторов присоединенных к сети, включая DR и маска сети, за которую отвечает DR
5. External - LSA 5 типа . Создаётся на ASBR для внешних маршрутов внедренных в OSPF.

**Нюансы**
1. Если мы изначально делаем соседство с соседнем маршрутизатором зная, что за интерфесом не будет коммутатора (тоесть он не будет в широковещательном домене), то следует указать тип интерфейса как p2p. Это позволит скоратить время ожидания установления сосседства, так как при широковещательном интерфейсе роутер будет ждать 40 секунд процесса выбора DR и BDR
2. Если у нас есть интерфейс, который смотрит в "конечную" сеть, то следует назначить такому интерфейсу режим passive , в этом случае он будет включен в процесс OSPF, но пакеты HELLO будут поавляться в его сторону, соответсвенно и отношение соседства налажено быть не может.
3. При указании интерфейсу режим Passive при флапанье линка маршрутизатор будет рассылать пакеты LSA 1 типа об изменнеи в топологии и соответсвенно будет постоянно перерасчитаваться ospf (ospf перерасчитывается при рассылке LSA 1 и 2 типа), для избежания такой ситуации можно редестрибьютить тупиковую сеть - в таком случае при флап линка будет разослан LSA 5 типа, что не видет к перерасчету OSPF
