...
_ _ _
Для устранения единой точки отказа на шлюзе по умолчанию применяется виртуальный маршрутизатор. Это обеспечивает имитацию работы одного маршрутизатора на узлах локальной сети. При совместном использование IP адреса и MAC-адреса два или более маршрутизатора могут работать как один виртуальный. Адрес IPv4 виртуального маршрутизатора настраивается в качестве шлюза по умолчанию для рабочих станций в отдельном сегменте IPv4
  
## HSSRP- проприетарный протокол Cisco, предназначенный для увеличения доступности маршрутизаторов выполняющих роль шлюза по умолчанию . Так же можно использовать "общий" проткол VRRP.